...
OpenDesk 插件是在 OpenDesk 主进程中运行的受信任 Node.js 模块，由 `pluginmgr` 应用负责发现、安装、启停、校验和加载。插件可以增加 Tool、Hook、Skill、MCP Server、TUI Command、应用事件监听器和子 Agent，也可以把这些能力组合在同一个插件中。
...
`setup` 失败会释放本次激活已创建的注册项。插件停用或宿主重载时，Hook、工具、Skill、MCP、命令、事件监听和子 Agent 都会随所属插件释放。
