...
~~~json
{
  ...
  "type": "module",
  "dependencies": {
    "@bitclub.ai/opendesk-plugin-sdk": "latest"
  }
}
~~~
...
| account | `ctx.accounts.registerBackend` 与 `ctx.accounts` 下的其他方法 |
| appearance | `ctx.appearance.setAppTitle`、`ctx.appearance.setAppDescription`、`ctx.appearance.setAppLogo`（0.3.7 起） |
| registry | `ctx.registry.registerSkillRegistry`、`ctx.registry.registerMcpRegistry` 与 `ctx.registry.fetch`（0.3.7 起） |
...
| agents | string[] | 子 Agent 目录的相对路径。目录可以直接包含 `agents.md`，也可以包含多个以 `agents.md` 为入口的一级子目录；详见[在插件中贡献子 Agent](/docs/dev/plugins/agents/) |
...
| accounts.registerBackend | function | 注册账号后端，`ctx.accounts` 下还提供网络、外链与凭据存取 |
| appearance.setAppTitle | function | 自定义应用标题（GUI 左上角品牌区，0.3.7 起） |
| appearance.setAppDescription | function | 自定义应用描述（左上角标题下方文案，0.3.7 起） |
| appearance.setAppLogo | function | 自定义应用 Logo（相对插件根目录的图片路径，0.3.7 起） |
| registry.fetch / registry.registerSkillRegistry / registry.registerMcpRegistry | function | 集成技能市场或 MCP 市场内容来源，fetch 继承宿主代理配置（0.3.7 起） |
...
- [在插件中添加工具](/docs/dev/plugins/tool/) — 把工具加入 Agent 可见工具集，支持参数 Schema、权限声明与 CLI/GUI 渲染器
- [在插件中挂载 MCP](/docs/dev/plugins/mcp/) — 注册本地 stdio 或远程 sse / streamableHttp MCP Server
- [在插件中添加 Skills](/docs/dev/plugins/skills/) — 注册可由用户或模型调用的 Skill 说明
- [在插件中使用钩子](/docs/dev/plugins/hooks/) — 在模型请求、会话审查、Skill 生命周期和工具调用等节点插入逻辑
- [在插件中贡献子 Agent](/docs/dev/plugins/agents/) — 声明可由主 Agent 调度的子 Agent，支持 agents.md 目录与动态注册
- [在插件中注入浏览器提示](/docs/dev/plugins/browser/) — 为特定站点向 use-browser 注入文本提示与可触发的页面动作
- [在插件中开发消息渠道](/docs/dev/plugins/channels/) — 接入 IM 平台收发消息，支持扫码/参数登录与结构化交互（权限审批 / askUser 卡片）
- [通过插件进行外观定制](/docs/dev/plugins/appearance/) — 自定义应用标题、描述与左上角 Logo，能力逐步开放中（0.3.7 起）
- [通过插件集成内容来源](/docs/dev/plugins/registry/) — 为技能市场与 MCP 管理器集成自定义内容来源（0.3.7 起）
- [扩展自定义命令](/docs/dev/plugins/commands/) — 注册可从 CLI、TUI 和 GUI 调用的命令
- [插件的调试与发布](/docs/dev/plugins/debug-publish/) — 配置、内置插件、安装与开发目录、校验与发布
