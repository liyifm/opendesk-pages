...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `--mode <mode>` | `opencode` | 支持 `opencode` 与 `acp` 两种模式。`acp` 模式详见 [通过 ACP 协议使用 OpenDesk](/docs/dev/acp-server-api/)。 |
...
| 类别 | 已实现 | 占位或未实现 |
| --- | --- | --- |
| 启动 | `serve --mode opencode` | `--mode acp`（已独立实现，见 [ACP 协议](/docs/dev/acp-server-api/)，不属于 opencode 兼容范围） |
